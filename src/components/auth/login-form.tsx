...
export function LoginForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.user) {
        const role = (result.user as typeof result.user & { role?: string })?.role;
  ...
  return (
    <Card className="overflow-hidden border border-nav-border/50 bg-panel/90 text-brand-foreground shadow-[0_25px_60px_rgba(2,6,23,0.75)]">
      <CardHeader className="space-y-3 text-center">
        <CardTitle className="text-3xl font-semibold">{t("loginFormTitle")}</CardTitle>
      ...
      <CardContent className="space-y-5">
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <Link href="/forgot-password" className="text-white/70 transition hover:text-white">
        ...
        <div className="space-y-4">
          ...
          <p className="text-center text-sm text-white/70">
            {t("loginNewToPixi")} {" "}
            <Link href="/signup" className="font-semibold text-white hover:text-brand-accent">
              {t("createAccountLink")}
            </Link>
          </p>
        </div>
